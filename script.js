...
function setupRemote() {
    ...
    conn.on('data', d => {
        ...
        // [MASTER_POWERS]: Ricezione poteri dal Master
...
// Funzioni Poteri Master (Esecuzione Visiva)
function triggerBlackout() {
    ...
    const mv = (e) => {
        ...
        ps.style.setProperty('--x', x + 'px');
        ps.style.setProperty('--y', y + 'px');
    };
    window.addEventListener('mousemove', mv);
    window.addEventListener('touchmove', mv);
    setTimeout(() => { 
        ps.classList.remove('blackout-mode'); 
...
function initGame() {
    ...
    // Mostra i poteri corretti in base al ruolo
    ...
    document.querySelectorAll('.btn-pwr').forEach(b => {
        b.disabled = amIMaster ? false : true; // Il master ha i poteri subito carichi
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        if(!amIMaster) {
            ...
            // Unlock progressivo poteri sfidante
...
function updateRankUI() {
    ...
    // [AUTO_SAVE]: Salva i punti ogni volta che cambiano
...
// Funzioni di supporto già presenti nel tuo codice originale
...
function useRescan() { 
    ...
    if(m.length) handleMove(m[Math.floor(Math.random()*m.length)]); 
}
