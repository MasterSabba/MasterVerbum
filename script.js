...
function updateRankUI() {
    ...
    if(mainLed) { mainLed.classList.add('led-on'); mainLed.style.background = "#39ff14"; }
...
function drawHangman() {
    ...
    ctx.lineWidth = 2.5;
    ctx.lineCap = "square"; // Linee più nette
...
function initGame() {
    ...
    // Reset pulsanti abilità (solo illuminazione, no pallini)
...
function triggerEnd(win) {
    ...
    if (!amIMaster) {
        if (win) {
            ...
            if (myHackerTag === "GUEST_USER") {
                let n = prompt("NAME:"); if(n) { myHackerTag = n; localStorage.setItem('mv_hacker_tag', n); }
            }
    ...
    // Fix layout per evitare che esca dal rettangolo
    t.style.fontSize = "clamp(1.2rem, 5vw, 2.2rem)";
    t.style.textAlign = "center";
    t.style.width = "90%";

    if (win) {
        t.style.color = "#00f2ff"; t.style.textShadow = "0 0 15px #00f2ff";
    } else {
        t.style.color = "#ff003c"; t.style.textShadow = "0 0 15px #ff003c";
    }
    document.getElementById('result-desc').innerHTML = `<p style="margin-top:15px;">KEY: <span style="color:#fff;">${secretWord}</span></p>`;
}

// --- RESET FUNZIONANTE ---
function resetAccount() {
    if(confirm("WIPE DATA?")) {
        localStorage.removeItem('mv_elite_stats');
        localStorage.removeItem('mv_hacker_tag');
        location.reload();
    }
}
